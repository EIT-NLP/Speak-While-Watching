...
class Qwen2VLPromptMixin:
    ...
    def set_dump_video(self, dump_video_func):
        self.dump_video_func = dump_video_func

    def dump_image(self, line, dataset):
        return self.dump_image_func(line)

    def dump_video(self, line, dataset):
        return self.dump_video_func(line)

    def use_custom_prompt(self, dataset: str) -> bool:
        return True

    def build_prompt(self, line, dataset: str) -> list[dict[str, str]]:
        if "activitynet" in dataset.lower():
            return self._build_activitynet_prompt(line, dataset)
        elif any(k in dataset.lower() for k in ["livesports", "2plus", "3_5"]):
            return self._build_livesports_prompt(line, dataset)
        elif "llava" in dataset.lower():
            return self._build_llava_video_prompt(line, dataset)
        else:
            return self._build_mmmu_prompt(line, dataset)

    def _build_activitynet_prompt(self, line, dataset: str) -> list[dict[str, str]]:
        MIN_PIXELS = 1280 * 28 * 28
        MAX_PIXELS = 5120 * 28 * 28
        tgt_path = self.dump_video(line, dataset)
        candidate_prompts = [
        ...
        if isinstance(tgt_path, list):
            msgs.extend([dict(type='video', value=p, min_pixels=MIN_PIXELS, max_pixels=MAX_PIXELS) for p in tgt_path])
        else:
            msgs = [dict(type='video', value=tgt_path, min_pixels=MIN_PIXELS, max_pixels=MAX_PIXELS)]
        msgs.append(dict(type='text', value=prompt))
        return msgs

    def _build_livesports_prompt(self, line, dataset: str) -> list[dict[str, str]]:
        MIN_PIXELS = 1280 * 28 * 28
        MAX_PIXELS = 5120 * 28 * 28
        tgt_path = self.dump_video(line, dataset)
        prompt = "What is the scene and activity shown in this video?"
        msgs = []
        msgs.append(dict(type='text', value=prompt))
        if isinstance(tgt_path, list):
            msgs.extend([dict(type='video', value=p, min_pixels=MIN_PIXELS, max_pixels=MAX_PIXELS) for p in tgt_path])
    ...
    def _build_llava_video_prompt(self, line, dataset: str) -> list[dict[str, str]]:
        MIN_PIXELS = 784
        MAX_PIXELS = 50176
        tgt_path = self.dump_video(line, dataset)
        candidate_prompts = [
