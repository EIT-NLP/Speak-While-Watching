...
def get_rope_index_25(
    ...
    if input_ids is not None and (
        ...
        for i, input_ids in enumerate(total_input_ids):
            ...
            # ---------- Variant-specific position id logic ----------
            # origin: assistant_aware=False, remaining tokens use continuous positions.
            # batch/group/gap: assistant_aware=True, split into pre-assistant text + generation.
            #   gap generation starts from 10000 (gap_num), group/batch starts from 0.
            if st < len(input_tokens):
                ...
                if assistant_aware:
                    try:
                        assistant_start = torch.where(input_ids == assistant_id)[0][0]
                        generation_tokens = input_tokens[assistant_start - 1 :]
                        text_len = assistant_start - st - 1
                        llm_pos_ids_list.append(
                            torch.arange(text_len).view(1, -1).expand(3, -1) + st_idx
                        )
                        # group/batch: generation positions start at 0; gap: start at 10000.
                        gap_num = 10000 if variant == "gap" else 0
                        llm_pos_ids_list.append(
                            torch.arange(len(generation_tokens)).view(1, -1).expand(3, -1) + gap_num
                        )
                    except IndexError:
...
def get_phrase_before_timestamp(text_stream, timestamp, video_start, start_from: int = 0):
    """Concatenate all segment words from start_from without timestamp filtering (same as batch/group logic)."""
    phrase = ''
...
def read_jsonl_line_linecache(path: str, index: int):
    line = linecache.getline(path, index + 1)  # linecache uses 1-based indexing.
    if line:
...
class LMMDataset(Dataset):
    def __init__(
        ...
        super().__init__()
        initial_fps_frames =        getattr(
            data_args, "initial_fps_frames", 6
        )
        streaming_fps_frames =      getattr(
            data_args, "streaming_fps_frames", 2
        )
        with_context =             getattr(
            data_args, "with_context", False
        )
        ...
        self.variant = getattr(data_args, "variant", "origin").lower()
        self.assistant_aware = self.variant in {"batch", "group", "gap"}
        self.stream_mode = "group" if self.variant in {"group", "gap"} else "batch"
        self.get_rope_index = functools.partial(get_rope_index_25, assistant_aware=self.assistant_aware, variant=self.variant)
    ...
    def preprocess_conversation_batch_stream(self, conversation: list):
        ...
        user_video_dict['video_end'] = user_video_dict['video_end']-user_video_dict['video_start']
        ...
        try:
            ...
            H, W = 224, 224  # Adjust spatial size to model-compatible defaults.
            epsilon = 1e-4
            fake_clip = np.full((n_frames, 3, H, W), 1.0 - epsilon, dtype=np.float32)  # White frames (T, C, H, W).
        ...
        phrase, next_start_from = get_phrase_before_timestamp(assistant_text_stream, clip_pts[-1],video_start) # Read all segments directly.
        conversation = [
            {
                'role': 'user', 'content': [
                    user_query_dict,
                    {'type': 'video', 'video': clip}, #{'type': 'video', 'video': clip[:self.initial_fps_frames]},
                ]
            },
        ]
        conversation_answer = [
    ...
    def preprocess_conversation_group_stream(self, conversation: list):
        ...
        for i in range(self.initial_fps_frames, len(clip), self.streaming_fps_frames):
            ...
            phrase, next_start_from = get_phrase_before_timestamp(assistant_text_stream, clip_pts[i + self.streaming_fps_frames-1], video_start, start_from=next_start_from)
    ...
    def getitem(self, index):
        try:
            ...
            import json
            import traceback
            print(f"[BAD SAMPLE] index={index} err={repr(e)}")
            try:
                raw = self.load_example(index)
                print("[BAD SAMPLE RAW]\n", json.dumps(raw, ensure_ascii=False, default=str)[:4000])
            except Exception as e2:
                print("[BAD SAMPLE RAW DUMP FAILED]", repr(e2))
            traceback.print_exc()
            raise

    def _getitem_impl(self, index):
        sample = self.load_example(index)
        QUESTION_LIST = [
            "Summarize the actions and events in the video.",
            "What actions can be observed in the video?",
            "Describe the key objects and movements in this video.",
            "What is the scene and activity shown in this video?"
        ]
        video_path = sample["video_path"]
        video_duration = sample["video_duration_in_s"]
        human_caption = sample["human_caption"].strip()
        video_start = 0.0
        ...
        conversation = [
            ...
            },
            {
                ...
                "content": [
                    {
                        "type": "text_stream",
                        "text_stream": [[video_start, video_end, human_caption]]
                    }
                ]
            }
        ]
        ...
        if special_process_for_stream:
            if self.stream_mode == "group":
                conversation, video_inputs = self.preprocess_conversation_group_stream(conversation)
            else:
                conversation, video_inputs = self.preprocess_conversation_batch_stream(conversation)
            image_inputs = None
        else:
            if not video_inputs and not image_inputs:
                image_inputs, video_inputs = process_vision_info(conversation)
        texts = self.processor.apply_chat_template(conversation, tokenize=False, add_generation_prompt=False, returns_tensors='pt')
...
if __name__ == "__main__":
    ...
    dataset = LMMDataset(
        annotation_paths=[
            os.getenv("QWEN2_5_VL_TRAIN_DATA", "dataset/train_3_5.jsonl"),
    ...
    dataloader = DataLoader(dataset, batch_size=1, shuffle=True, num_workers=1, collate_fn=dataset.data_collator)
