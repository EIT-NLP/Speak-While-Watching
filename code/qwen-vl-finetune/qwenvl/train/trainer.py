...
try:
    ...
    # transformers >= 4.53 removed ALL_LAYERNORM_LAYERS from trainer
...
def _flash_attention_forward_fallback(
    ...
    **kwargs,
    ...
    """Fallback using PyTorch scaled_dot_product_attention when flash_attn is not available."""
    ...
    for i in range(cu_seqlens.size(0) - 1):
        ...
        out = torch.nn.functional.scaled_dot_product_attention(
            q, k, v, is_causal=causal, dropout_p=dropout if torch.is_grad_enabled() else 0.0
        )
    ...
    **kwargs,
    ...
    if not FLASH_ATTN_AVAILABLE:
        return _flash_attention_forward_fallback(
            ...
            query_length, is_causal, dropout=dropout, use_top_left_mask=use_top_left_mask, **kwargs
        )
    assert query_states.size(0) == key_states.size(0) == value_states.size(0) == 1
    query_states = query_states.squeeze(0)
    key_states = key_states.squeeze(0)
...
def replace_qwen2_vl_attention_class():
    """Replace flash attention with custom varlen implementation. No-op when flash_attn is not installed."""
    if not FLASH_ATTN_AVAILABLE:
        return
    import transformers
    ...
    # Add the code root directory to Python path.
    code_root = Path(__file__).resolve().parents[3]
    sys.path.append(str(code_root))
    import model_code.modeling_qwen2_5_vl
    # from .... import model_code
    model_code.modeling_qwen2_5_vl._flash_attention_forward = (
        _flash_attention_forward
    )
    model_code.modeling_qwen2_5_vl.Qwen2_5_VLModel._update_causal_mask = (
        _update_causal_mask
    )
