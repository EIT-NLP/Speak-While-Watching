...
from .causal_mask import (
    build_group_custom_causal_mask_for_eval,
    build_group_custom_causal_mask_for_train,
)
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def forward(
        ...
        cache_position: Optional[torch.LongTensor] = None,
        input_ids_for_group: Optional[torch.LongTensor] = None,
        video_grid_thw: Optional[torch.LongTensor] = None,
        token_len_schedule: Optional[List[int]] = None,
        ...
        causal_mask = self._update_causal_mask(
            ...
            past_key_values,
            output_attentions,
            input_ids_for_group=input_ids_for_group,
    ...
    def _update_causal_mask(
        ...
        variant = getattr(self.config, "variant", os.getenv("QWEN2_5_VL_VARIANT", "origin")).lower()
        use_group_mask = variant in {"group", "gap", "overlap"}
        is_decoding = input_ids_for_group is not None and input_ids_for_group.shape[1] == 1

        if use_group_mask and input_ids_for_group is not None and video_grid_thw is not None and not is_decoding:
            try:
                video_frame_token_counts = []
                for b in range(video_grid_thw.shape[0]):
                    _, h, w = video_grid_thw[b]
                    video_frame_token_counts.append((h // 2) * (w // 2))

                if self.training:
                    dataset_use = getattr(self, "dataset_use", None)
                    causal_mask = build_group_custom_causal_mask_for_train(
                        input_ids=input_ids_for_group,
                    ...
                    causal_mask = build_group_custom_causal_mask_for_eval(
                        input_ids=input_ids_for_group,
                        video_frame_token_counts=video_frame_token_counts,
                        ANSWER_GROUP_SIZE=3,
                        token_len_schedule=token_len_schedule,
                ...
                causal_mask = _default_causal_mask()
        else:
            causal_mask = _default_causal_mask()
...
class Qwen2_5_VLForConditionalGeneration(Qwen2_5_VLPreTrainedModel, GenerationMixin):
    ...
    def get_rope_index(
        ...
        Note (variant alignment):
        - Origin (modeling_qwen2_5_vl.py): device = second_per_grid_ts.device at top; no fallback.
        - Group/gap/overlap for_train & for_eval: device = second_per_grid_t.device inside the loop (per segment).
        - Position computation (time_tensor = expanded_range * second_per_grid_t * tokens_per_second, and the
          "if st < len(input_tokens)" remainder block) is identical across origin/group/gap/overlap; only device
          resolution differs. Here we use a single implementation with device resolved at top via fallback
          (second_per_grid_ts -> video_grid_thw -> image_grid_thw -> input_ids -> self.parameters()) so it works
          when second_per_grid_ts is None (e.g. decode) or when image appears before video.
        - Variant-specific position id behavior is implemented in the training data pipeline only:
          see qwenvl/data/lmm_dataset.py, get_rope_index_25, branch "if st < len(input_tokens)"
          (assistant_aware + gap_num).
          During training, each sample already carries position_ids from
          LMMDataset.getitem -> get_rope_index. This model-side get_rope_index is used only
          when forward/generation is called without provided position_ids, and does not branch
          by variant.
        ...
        if second_per_grid_ts is not None:
            device = second_per_grid_ts.device
    ...
    def _validate_model_kwargs(self, model_kwargs, *args, **kwargs):
        # token_len_schedule is used in group/gap/overlap evaluation.
        # Forward accepts it, but base class validation may not; pop then restore.
        token_len_schedule = model_kwargs.pop("token_len_schedule", None)
        super()._validate_model_kwargs(model_kwargs, *args, **kwargs)
        if token_len_schedule is not None:
            model_kwargs["token_len_schedule"] = token_len_schedule
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs,
        ...
        # Ensure token_len_schedule is forwarded at every generation step for group/gap/overlap.
        if token_len_schedule is not None:
            model_inputs["token_len_schedule"] = token_len_schedule
