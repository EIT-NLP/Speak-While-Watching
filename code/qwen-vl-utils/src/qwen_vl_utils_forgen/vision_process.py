...
def process_vision_info_forgen(
    conversations: list[dict] | list[list[dict]],
    video_start: float,
    video_end: float,
    return_video_kwargs: bool = False,
    ...
    for vision_info in vision_infos:
        if "image" in vision_info or "image_url" in vision_info:
            ...
            # video_input, video_sample_fps = fetch_video(vision_info, return_video_sample_fps=True)
            video_input, video_sample_fps = _read_video_decord_plus({'video': vision_info['video'][7:], 'video_start': video_start, 'video_end': video_end, 'remote_loader': None})
            video_input = _spatial_resize_video(video_input)
            video_sample_fps_list.append(video_sample_fps)
            video_inputs.append(video_input)
        else:
            raise ValueError("image, image_url or video should in content.")
